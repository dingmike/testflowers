...
class OutIntegralIn extends Component{
  componentDidMount(){
  ...
  render(){
    ...
    const { loading, height, outIntegral=[], pagination:{
      total=0,
      page=1,
      pageSize=10
    }} = integral;
          ...
          { 
            outIntegral.map((re,index)=>
              ...
              >
                ...
                <div className={ styles.header }>
                  <p className={ styles.headerName }>对方账号：{re.transferencePhone}</p>
                  <p className={ styles.headerNumber } style={{
                    color :"#9D0F14"
                  }}>-{re.money}</p>
                </div>
...
export default connect(({integral})=>({integral}))(OutIntegralIn)