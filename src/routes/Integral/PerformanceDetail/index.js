...
class Recharges extends Component{
  ...
  render(){
    ...
    const { loading, height, performanceInte, pagination:{
      isLast=false,
      page=1,
      type=2, //1-余额 2-积分 3-全部
      pageSize=10
    }} = cash;
    return (
      <StickyContainer>
        ...
        <PullToRefresh
          ...
          onRefresh={()=>{
            if(isLast){
              ...
              dispatch({ type : 'cash/queryMorePerformanceInte', payload : { nextPage : page + 1, pageSize: pageSize, type }})
            }
          }}
        >
          {
            performanceInte.map((re,index)=>
              <div
